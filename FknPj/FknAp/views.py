...
def send_notification(registration_ids, message_title, message_desc, post_id):
    ...
    fcm_api = (
        "AAAAkV-gc5c:APA91bF4PJPVDpihuGhCzMljtG1RjI-"
        "ZOn0xLr8UscqsQGw6nPZ7mDz9ttTeXZUj6LHjT1fdwkhUEdXYa22jR-"
        "dJ-OEr3_MDwTbVNUsTB8Wofl8H8ApQ8Sbo8dkEnFNTR5OXeOIrtKTS"
    )
...
def create_post(request):
    if request.method == "POST":
        ...
        post = Post.objects.create(
            creater=request.user, content_text=content_text
        )
        post_id = post.id
        try:
            ...
            if registration_ids:
                ...
                send_notification(
                    registration_ids, message_title, message_desc, post_id
                )
                print(
                    "Notification sent to {} devices.".format(
            ...
            print(
                "An error occurred: FCMDevice model misconfigured."
            )
...
def comments(request, post_id):
    ...
    if request.method == "POST":
        ...
        Notification.objects.create(
            user=request.user, post=post, message=message
        )
